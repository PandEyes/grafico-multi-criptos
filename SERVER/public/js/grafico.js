...
function totalChart(ctx, extraerdatos){
    const precio = extraerdatos.map((item) => {
        if(item.precio > 0){
            return item.precio;
        }
    })
    
    const pesimista = extraerdatos.map((item) => {
        if(item.pesimista > 0){
            return item.pesimista;
        }
    })
    
    const optimista = extraerdatos.map((item) => {
        if(item.optimista > 0){
    ...
    var total = []
    /*global Chart*/
    if (precio.length >= pesimista.length && precio.length >= optimista.length) {
        for (let i = 0; i < precio.length; i++) {
           total.push({
                type: 'bar',
                label: '',
                data: [precio[i], pesimista[i], optimista[i]]
           })            
        }
    } else if (pesimista.length >= precio.length && pesimista.length >= optimista.length){
        for (let i = 0; i < pesimista.length; i++) {
           total.push({
                type: 'bar',
                label: '',
                data: [precio[i], pesimista[i], optimista[i]]
        ...
        for (let i = 0; i < optimista.length; i++) {
           total.push({
                type: 'bar',
                label: '',
                data: [precio[i], pesimista[i], optimista[i]]
    ...
    const chart = new Chart(ctx, {
        data: {
            datasets: total,
            labels: ['Pesimista', 'Precio Real', 'Optimista'],
        },
        plugins: [plugin],
        options: {
            layout: {
